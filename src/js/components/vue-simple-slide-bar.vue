<template>
  <div
    ref="wrap"
    class="vue-simple-slide-bar-component vue-simple-slide-bar-horizontal"
    :style="innerWrapStyles"
    @click="wrapClick"
  >
    <div
      ref="elem"
      class="vue-simple-slide-bar"
      :style="trackStyles"
    >
      <div
        ref="process"
        class="vue-simple-slide-bar-process"
        :style="processStyles"
      ></div>
      <template>
        <div
          ref="tooltip"
          class="vue-simple-slide-bar-always vue-simple-slide-bar-tooltip-container"
        ...
        >
          <span
            v-if="showTooltip"
            class="vue-simple-slide-bar-tooltip-top vue-simple-slide-bar-tooltip-wrap"
          >
            <slot name="tooltip">
              <span
                class="vue-simple-slide-bar-tooltip"
                :style="tooltipStyles"
              >{{ val }}</span>
            </slot>
          </span>
      ...
      <div
        v-show="showBean"
        ref="bean"
        class="vue-simple-slide-bar-bean"
        :style="beanStyles"
        @mousedown="moveStart"
        @touchstart="moveStart"
      ></div>
    ...
    <div
      v-if="range"
      class="vue-simple-slide-bar-range"
    >
      <div
        v-for="(r, index) in range"
        :key="index"
        class="vue-simple-slide-bar-separate"
        :style="dataLabelStyles"
      >
        <span
          v-if="!r.isHide"
          class="vue-simple-slide-bar-separate-text"
        >
          {{ r.label }}
        </span>
      </div>
...
export default {
  ...
  computed: {
    ...
    innerWrapStyles () {
      return Object.assign({}, { 'padding': '10px 0', 'min-height': this.range ? '100px' : undefined }, this.wrapStyles);
    }
  },
  ...
  methods: {
    ...
    setTransform (val) {
      let toolTipX = val - ((this.$refs.tooltip.scrollWidth - 2) / 2);
      let translateToolTip = `translateX(${toolTipX}px)`;
      ...
      let beanX = val - ((this.$refs.bean.scrollWidth - 2) / 2);
      let translateBean = `translateX(${beanX}px)`;
